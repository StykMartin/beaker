...
def inject_args(func, injections, args, kw, start=0):
    ...
    for argname, argval in six.iteritems(injections):
...
def recursive_update(to_dict, from_dict):
    ...
    for k, v in six.iteritems(from_dict):
...
def flatten_sequence(seq):
    """Flatten sequence."""
    for item in seq:
        if isSequenceType(item) and not isinstance(item, six.string_types):
...
def pop_from_nested_dict(params, keys):
    """Pop a value from a nested parameter dictionary.

    You must pass a list or dotted key as the key chain to the value.
    If a value for this key chain is found, it will be removed and
    the value will be returned. If this creates empty subkeys,
    these will also be removed. If no single value is associated
    ...
    if isinstance(params, dict):
        ...
        value = params
    for key, params in reversed(zip(keys, values)):
        del params[key]
        if params:
            break
    return value
...
def pop_request_params(keys):
    """Pop a value from the nested request parameter dictionary."""
    return pop_from_nested_dict(request.params, keys)
    ...
    if isinstance(value, str):
        ...
        try:
            ...
            try:
                ...
                # fail early
                raise ValueError("Non-six.text_type string: %r" % value)
    return value
...
def fixentities(htmltext):
    ...
    """
    def repl(matchobj):
        entity = html_entities.entitydefs.get(matchobj.group(1).lower())
        if not entity:
...
__all__ = ["Bunch", "DictObj", "DictWrapper", "Enum", "setlike",
           ...
           "flatten_sequence", "pop_from_nested_dict",
           "pop_request_params", "load_class",
